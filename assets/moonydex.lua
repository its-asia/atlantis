-- stolen from some skidded script on v3rm
-- slightly modified
...
local getelysianpath = getelysianpath or function() -- does anyone use elysian anymore?
	return 'dex/'
end

local Tell = Notify and function(Text)
	Notify(nil, 'dex', Text)
end or printconsole and function(Text)
	printconsole(Text, 255, 255, 255)
end or warn

local RbxApi = _G.RbxApi
if not RbxApi then
	Tell('rbxapi is not loaded?')
...
local GetSelection, SetSelection
local D_E_X = CreateGui()

D_E_X.Parent = Atlantis and Atlantis.Gui.Parent or game:GetService('CoreGui')
...
spawn(function()
	...
	Mouse.Button1Down:connect(function()
		if not SetSelection then return end
	...
	SaveMapButton.MouseButton1Click:connect(function()
		...
		if lightingProperties then
			...
			saveProp(Lighting,"Ambient",lightingProps)
			saveProp(Lighting,"Brightness",lightingProps)
			saveProp(Lighting,"ColorShift_Bottom",lightingProps)
			saveProp(Lighting,"ColorShift_Top",lightingProps)
			saveProp(Lighting,"GlobalShadows",lightingProps)
			saveProp(Lighting,"OutdoorAmbient",lightingProps)
			saveProp(Lighting,"GeographicLatitude",lightingProps)
			saveProp(Lighting,"TimeOfDay",lightingProps)
			saveProp(Lighting,"FogColor",lightingProps)
			saveProp(Lighting,"FogEnd",lightingProps)
			saveProp(Lighting,"FogStart",lightingProps)
		end
...
- Drag objects to reparent
	...
	local Selection do
		...
		SetSelection = bindSetSelection
		GetSelection = bindGetSelection
	end
	...
	function rightClickMenu(sObj)
		local mouse = LocalPlayer:GetMouse()
 -- cut is kept for the 1 nerd who uses cut to delete lol
		currentRightClickMenu = CreateRightClickMenu(
			{"Cut","Delete","Select Children","Teleport To",'Copy Path',"View Script","Save Instance","Call Remote",'Claim','View','Unview'},
			"",
			false,
			function(option)
				if option == "Delete" or option == "Cut" then
				...
				elseif option == "Teleport To" then
					...
					for i = 1,#list do
						if list[i]:IsA("BasePart") then
							pcall(function()
								LocalPlayer.Character.HumanoidRootPart.CFrame = list[i].CFrame
							end)
							break
						end
					end
				elseif option == "View" then
					...
					for i = 1,#list do
						pcall(function()
							workspace.CurrentCamera.CameraSubject = list[i]
						end)
						break
					end
				elseif option == "Unview" then
					if not Option.Modifiable then return end
					local list = Selection:Get()
					for i = 1,#list do
						pcall(function()
				...
				elseif option == "Claim" then
					if not Option.Modifiable then return end
					local list = Selection:Get()
					for i = 1,#list do
						if list[i]:IsA("BasePart") then
							pcall(function()
								SetNetworkOwner(list[i], LocalPlayer)
							end)
							break
						end
					end
				elseif option == "Copy Path" then
					if not Option.Modifiable then return end
					local GetInstancePath = GetInstancePath or function(x) return x:GetFullName() end
					
					local list = Selection:Get()
					for i = 1,#list do
						if list[i]:IsA("BasePart") then
							pcall(function()
								setclipboard(GetInstancePath(list[i]))
							end)
							break
						end
					end
		...
		for i,v in pairs(GetSelection()) do
			if CanEditProperty(v,propertyData) then
				pcall(function()
					...
					ChangeProperty(object, propertyName, value)
				end)
			end
		end
	end
	...
	function showProperties(obj)
		...
		for _,nextObj in pairs(obj) do
			if not foundProps[nextObj.className] then
				...
				for i,v in pairs(RbxApi.GetProperties(nextObj.className)) do -- XD XD XD XD XD XD XD FUNNY
